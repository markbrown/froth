...
| `close` | `( env body -- closure )` | Create closure: `(map func -- closureval)` or `(array int -- bytecodeval)` |
| `closureEnv` | `( closure -- env )` | Extract env (map) from closureval or context (array) from bytecodeval |
| `closureBody` | `( closure -- body )` | Extract body (func) from closureval or code address (int) from bytecodeval |
